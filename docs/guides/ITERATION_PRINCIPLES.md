...
6. **执行层止损（双预算制）**：同一假设/同一故障的连续修复尝试 ≥3 次 → 停止，落一条 `kb/lessons/` 或 ADJ 工件后换方向或转 DORMANT；任何一次成功清零连续计数。此规则治理 agent 调试会话，与实验层预注册止损（learnability gate）互补（ADJ-014，原号 ADJ-012 撞号已勘误）。
7. **教训层 kb/lessons/**：跨项目可复用的行为教训沉淀为"触发条件→对策→来源"结构条目，§4.2 异常晋升以它为匹配依据；准入判据 = "删掉这条会再犯同类错吗"；演进纪律（每窗口 ≤4 操作，ADD/EDIT/REMOVE）见 kb/README（ADJ-014，原号 ADJ-012 撞号已勘误）。
