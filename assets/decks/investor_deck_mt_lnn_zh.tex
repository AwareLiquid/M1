...
\begin{itemize}
    ...
    \item \textbf{内置解释性与跳过机制：} 提供可选的稀疏共振算力门控（Sparse-Resonance Compute Gating），以小幅精度预算换取实测 CPU 吞吐提升。内置因果链与自我监控提取头，使内部推理步骤可被直接检视。
\end{itemize}
\end{column}
...
\begin{frame}{从“路人观光客”到“本地向导”：内化的脑内物理引擎}
\begin{itemize}
    \item \textbf{之前的毛病（极度依赖外挂工具）：} 传统模型处理空间路线问题，就像只看风景照的观光客。必须先把环境拍成照片或转成文字，如果换个视角就直接“脸盲”（破对称性）。真要找路或测距，还得频繁调用导航API，反应卡顿，常出现“说头头是道，但在真实空间撞墙”的尴尬。
    \item \textbf{改完的强悍之处（潜空间自主模拟）：} 我们把推演地形、空间物理机制直接塞进了模型内部！模型现在能真正像人类一样，在脑海里“想象”和模拟物理轨迹。它不需要外挂软件帮忙，自己就能在内部进行3D拓扑分析、距离判断和躲避障碍规划。
    \item \textbf{时空强强联合（大小场景通吃）：} 原本 MT-LNN 就是专攻“连续变化”的液态架构，现在不仅算得快，更是连小到室内桌面、大到城市路网分析的动态场景都能零延迟拿捏。低算力也可以流畅部署，是攻占机器人和自动驾驶市场最难啃的“空间直觉板块”！
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{动态算力跳过 (Compute Skipping)：} 仿生大脑，在面对高度冗余或简单的上下文区间时，可选地令休眠的原丝通道被掩码跳过计算（Masked out），以小幅精度预算换取实测的 CPU 算力节约（详见稀疏共振消融）。
...
\begin{itemize}
    ...
    \item \textbf{迈向全天候流式处理：} 骨干网络与模态无关——已开放\textbf{多模态注入通路}（视觉 patch 与特征投影前端将非文本流融合进 token 序列）。面对长流输入，MT-LNN 依靠动态液态状态消化负载，而非依赖不断膨胀的缓存。（预训练视觉/音频塔为路线图项目，尚未交付。）
\end{itemize}
\end{frame}
...
\begin{frame}{当前进展：可运行的完整技术栈，而不止是论文}
\begin{itemize}
    \item \textbf{超越白板阶段：} 架构已端到端实现，具备原生自回归 \texttt{generate()}（KV-cache 解码、top-k/top-p 采样）及可交互的 Python/Gradio 沙盒前端。
    \item \textbf{服务化与打包：} 提供 OpenAI 风格的 FastAPI 服务（completions + SSE 流式）与 Docker 镜像，可离线、私有化部署于普通 PC 或企业内网，无需任何外部 API 依赖。
    \item \textbf{由测试背书：} 280+ 项自动化测试覆盖骨干网络、增量解码、文本生成、服务化与多模态注入通路。（RAG / 检索集成为路线图项目，尚未交付。）
\end{itemize}
\end{frame}
...
\begin{enumerate}
    ...
    \vspace{0.5em}
    \item \textbf{三阶段规模化与多模态基座 (未来 18 个月以上)：} 与模态无关的注入通路已就位；本阶段在其之上训练预训练视觉/音频塔，并使用超千亿数据集参与大规模联合训练，用极致常数时间效能推动更高度的泛化理解引擎底座。
\end{enumerate}
\end{frame}
