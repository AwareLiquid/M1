...
\begin{frame}{近期能力升级 1：空间计算 —— 从"读懂文字"到"理解空间"}
\small
\begin{columns}
\begin{column}{0.58\textwidth}
\textbf{新增四层空间认知栈}（受海马-内嗅皮层 Bicanski \& Burgess 2021 启发）：
\begin{itemize}\itemsep=0.25em
    \item \textbf{L1 网格/位置细胞编码：}把连续坐标转成类脑空间群码，回答"我在哪"。
    \item \textbf{L2 位置索引联想记忆：}沿轨迹"边走边记"，之后仅凭\textbf{近似位置}即可\textbf{模式补全}召回当时内容——零可训练参数、$O(1)$ 内存、可跨会话持久化。
    \item \textbf{L3 因果激活引导：}把空间信念稳定在认知流形上，抗长程漂移。
    \item \textbf{L4 世界模型推演（路线图）：}将空间动力学向前滚动，做"想象/预演"。
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{产品价值：}
\begin{itemize}\itemsep=0.25em
    \item 从纯文本扩展到\textbf{具身/空间智能}：机器人巡航、AR/VR 导航、IoT 空间巡检、"空间 RAG"。
    \item 端侧\textbf{零额外参数}即获一张类脑\textbf{认知地图}，与 $O(1)$ 主干同源——不增显存、不增成本。
    \item 与文本骨干\textbf{完全解耦}，可作为独立前端即插即用。
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{近期能力升级 2：可验证的数学算子代数 —— 把"黑箱"变"可证明"}
\small
\begin{columns}
\begin{column}{0.58\textwidth}
\textbf{新增 11 个零参数、与主干完全解耦的算子模块}（吸引子 / 几何 / STDP / 拓扑 / 物理 / 声学 / 空间 / 摄入 / 回放 / 持续评估 / 显著事件），每个模块的行为契约都由\textbf{属性测试钉死}。
\begin{itemize}\itemsep=0.25em
    \item \textbf{信息几何：}在概率单纯形上引入 \textbf{Fisher-Rao 度量}（闭式球面等距），让空间码的比较/插值/平均\textbf{度量自洽}，消除欧氏度量在弯曲流形上的误差累积。
    \item \textbf{配套数学白皮书：}分三档\textbf{诚实标注}"已证明"的含义——闭式恒等式 / 属性测试钉定 / 统计场景，不夸大。
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{产品价值：}
\begin{itemize}\itemsep=0.25em
    \item \textbf{企业级可信赖 = 护城河。}不是调参玄学，而是可审计、可复现的算子代数。
    \item \textbf{967 项测试常驻绿灯}、CPU 两分钟内全过，每条能力主张都有脚本与产物背书。
    \item 金融/法律/政企等高合规场景的\textbf{硬通货}：行为可被第三方逐条验证。
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{近期能力升级 3：持续学习 —— 端侧"越用越聪明"且不遗忘}
\small
\begin{columns}
\begin{column}{0.58\textwidth}
\textbf{新增抗灾难性遗忘的持续学习栈}（受海马回放 Rolnick 2019 启发）：
\begin{itemize}\itemsep=0.25em
    \item \textbf{有界经验回放：}固定内存、对数据流均匀采样的复习缓冲区——无需知道流长度，把过去样本与实时流交织，喂回模型对抗遗忘。
    \item \textbf{持续学习度量：}从一张准确率矩阵算出\textbf{遗忘 / 保持 / 迁移}三项指标，让"学了新的会不会忘旧的"\textbf{可量化}。
    \item \textbf{流式演示：}同一数据流下\textbf{回放 vs 灾难性遗忘}的对照实验，行为由测试钉死。
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{产品价值：}
\begin{itemize}\itemsep=0.25em
    \item \textbf{端侧终身学习}：设备本地随用户持续个性化，且不遗忘已掌握的能力——隐私不出端、模型越用越贴合。
    \item 直击大模型"微调即灾难性遗忘"的行业痛点，是边缘个性化的\textbf{差异化壁垒}。
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{近期能力升级 4：全脑闭环 —— 可运行的自主认知智能体}
\small
\textbf{新增端到端整合演示}（\texttt{examples/demo\_cognitive\_agent.py}）：把类脑各层串成\textbf{一个闭环}求解 2D 导航任务，每一环都是\textbf{真实模块、无任何 Mock}：
\begin{itemize}\itemsep=0.25em
    \item \textbf{感知}（L1 网格细胞）$\rightarrow$ \textbf{记忆}（L2 空间认知地图，模糊线索模式补全召回）$\rightarrow$ \textbf{注意}（自上而下目标调制：门关=严格空操作，门开=引导主干）$\rightarrow$ \textbf{想象}（L4 潜在推演，零参数、置信度随步衰减）$\rightarrow$ \textbf{行动}。
    \item \textbf{产品价值：}从"一堆能力"跃迁为\textbf{具身智能体的认知闭环}——把"感知-记忆-推演-决策"的愿景变成\textbf{可运行}的代码。（当前为整合演示原型，非生产成品。）
\end{itemize}
\end{frame}

\begin{frame}{近期能力升级 5：从能力到产品 —— 双速哨兵 + 生产化部署}
\small
\begin{columns}
\begin{column}{0.56\textwidth}
\textbf{双速哨兵 (DualSpeedSentry) 商业闭环}——把算子层、世界模型、失效保护、显著事件触发器串成一条产品级回路：
\begin{itemize}\itemsep=0.2em
    \item \textbf{感知}（双耳声学定位 ITD/多普勒/测距）$\rightarrow$\textbf{推理}（目标是否在保护区内，每拍重算不存储）$\rightarrow$\textbf{预测}（一步弹道预报产生"惊讶度"）$\rightarrow$\textbf{触发}（仅在真正状态变化时\textbf{唤醒}慢/符号/云层，绝不轮询热循环）$\rightarrow$\textbf{断连续航}（信号中断时按信任预算决定续推或安全停机）$\rightarrow$\textbf{安全执行}（断路器保证执行输出\textbf{恒为有限、在机械限幅内}）。
\end{itemize}
\end{column}
\begin{column}{0.44\textwidth}
\textbf{生产化部署：}
\begin{itemize}\itemsep=0.2em
    \item HF 适配器\textbf{服务化路径}（带契约测试）+ 容器打包与运维工具——\textbf{可直接部署}。
\end{itemize}
\vspace{0.4em}
\textbf{产品价值：}
\begin{itemize}\itemsep=0.2em
    \item 一条可落地的\textbf{安全关键 / 监测哨兵}垂直产品线（快反射 + 慢审议双速）。
    \item 失效保护可证明有界，契合\textbf{工业 / 安防 / 国防}的可靠性硬门槛。
\end{itemize}
\end{column}
\end{columns}
\end{frame}
