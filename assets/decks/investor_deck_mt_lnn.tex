...
\begin{itemize}
    ...
    \item \textbf{Replacing Static FFNs:} The architecture integrates 13 parallel, continuous-time liquid network pathways with \textit{optional sparse-resonance compute gating}.
    \item \textbf{Conscious-Computing Instrumentation:} The backbone generates language while natively tracking an integrated-information proxy ($\hat{\Phi}$) under an \textit{Anesthesia Validation Protocol} --- a quantitative, reproducible probe of representational collapse (not a solved consciousness claim).
\end{itemize}
\end{column}
...
\begin{itemize}
    ...
    \item \textbf{Sparse-Resonance Compute Gating:} Optional dynamic $\kappa$ scale-gates can deactivate dormant channels mid-computation, trading a small accuracy budget for \textbf{measured CPU throughput gains} (see the sparse-resonance ablation).
    \item \textbf{Explainable Causal Logging:} Moving away from black boxes. MT-LNN state matrices are parsed by \textit{Causal Chain} and \textit{Self-Monitor} extraction heads, making internal logical stepping directly inspectable.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{Toward ``Always-On'' Intelligence:} The backbone is modality-agnostic --- it exposes a \textit{multimodal injection path} (vision-patch and feature-projector frontends fuse non-text streams into the token sequence). Where standard models suffer memory cascades on long streams, MT-LNN dissipates the load via dynamic liquid states rather than an ever-growing cache. (Pretrained vision/audio towers are a roadmap item.)
\end{itemize}
\end{frame}
...
\begin{frame}{Current Status: A Runnable Stack, Not Just a Paper}
\begin{itemize}
    \item \textbf{Beyond the whiteboard:} The architecture is implemented end-to-end with native autoregressive \texttt{generate()} (KV-cache decoding, top-k/top-p sampling) and an interactive Python/Gradio sandbox UI.
    \item \textbf{Serving \& packaging:} An OpenAI-style FastAPI server (completions + SSE streaming) and a Docker image package the model for offline, on-prem deployment on a standard PC or enterprise intranet --- no external API dependency.
    \item \textbf{Validated by tests:} 280+ automated tests cover the backbone, incremental decoding, generation, serving, and the multimodal injection path. (RAG / retrieval integration is a roadmap item, not yet shipped.)
\end{itemize}
\end{frame}
...
\begin{enumerate}
    ...
    \vspace{0.4em}
    \item \textbf{Phase 3: Scale \& Multimodal (Months 6-18):} Scale on an H100 cluster. The modality-agnostic injection path is already in place; Phase 3 trains the pretrained vision/audio towers on top of it and pushes toward 100B+ parameter scale to challenge legacy Transformers across global LLM arenas.
\end{enumerate}
\end{frame}
