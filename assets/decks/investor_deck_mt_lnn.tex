...
\begin{itemize}\itemsep=0.3em
    ...
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{4. Language-Modeling Efficiency (from-scratch, matched parameter class)}
    \begin{itemize}
        \item \textbf{Testing Baseline:} MT-LNN vs.\ a matched Transformer and a Mamba baseline, all trained from scratch on WikiText-103 under an identical fp32 budget (2000 steps, single seed).
        \item \textbf{Commercial Meaning in Plain Terms:} At the same $\sim$125M scale, MT-LNN reaches a \textbf{$\sim$31\% lower perplexity than the Transformer} (257 vs 371) and \textbf{$\sim$38\% lower than Mamba} (257 vs 414) with fewer parameters --- the first head-to-head where the architecture beats the current leading efficient-attention alternative. In plain terms: more capability per parameter, which compounds directly into lower training and serving cost. \textit{(Single-seed, budget-limited signal; Mamba is width/depth-mismatched, an external reference rather than an architecture-matched control.)}
