...
\begin{frame}{Recent Upgrade 1: Spatial Computing --- From Reading Text to Understanding Space}
\small
\begin{columns}
\begin{column}{0.58\textwidth}
\textbf{A new four-layer spatial-cognition stack} (inspired by the hippocampal-entorhinal model of Bicanski \& Burgess 2021):
\begin{itemize}\itemsep=0.25em
    \item \textbf{L1 Grid/Place-cell encoding:} turns continuous coordinates into a brain-like spatial population code --- answering ``where am I''.
    \item \textbf{L2 Place-indexed associative memory:} write what you perceive along a trajectory, then recall it later from an \textbf{approximate position} via \textbf{pattern completion} --- \textbf{zero trainable parameters}, $O(1)$ memory, persists across sessions.
    \item \textbf{L3 Causal activation steering:} keeps the spatial belief on its cognitive manifold, resisting long-range drift.
    \item \textbf{L4 World-model simulation (roadmap):} roll the spatial dynamics forward to ``imagine / pre-plan''.
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{Product value:}
\begin{itemize}\itemsep=0.25em
    \item Extends from text-only to \textbf{embodied / spatial intelligence}: robot navigation, AR/VR wayfinding, IoT spatial inspection, ``spatial RAG''.
    \item An on-device, brain-like \textbf{cognitive map} at \textbf{zero extra parameters} --- same $O(1)$ DNA as the backbone: no added VRAM, no added cost.
    \item \textbf{Fully decoupled} from the text backbone --- drops in as a standalone front-end.
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Recent Upgrade 2: A Verifiable Operator Algebra --- Turning the Black Box into Provable Math}
\small
\begin{columns}
\begin{column}{0.58\textwidth}
\textbf{11 new zero-parameter, fully backbone-decoupled operator modules} (attractor / geometry / STDP / topology / physics / acoustics / spatial / ingest / replay / continual-eval / salience), each with a behavioural contract \textbf{pinned by property tests}.
\begin{itemize}\itemsep=0.25em
    \item \textbf{Information geometry:} a \textbf{Fisher-Rao metric} on the probability simplex (closed-form sphere isometry) makes comparing / interpolating / averaging spatial codes \textbf{metric-consistent}, killing the error accumulation a Euclidean metric incurs on a curved manifold.
    \item \textbf{A companion math white paper} that \textbf{honestly grades} what ``proven'' means --- closed-form identity / property-test-pinned / statistical scenario --- with no over-claiming.
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{Product value:}
\begin{itemize}\itemsep=0.25em
    \item \textbf{Enterprise-grade trust = the moat.} Not hyperparameter alchemy, but an auditable, reproducible operator algebra.
    \item \textbf{967 tests permanently green}, all passing on CPU in under 2 minutes --- every capability claim is backed by a script and an artefact.
    \item A hard currency for \textbf{high-compliance} finance / legal / public-sector deployments: behaviour verifiable line-by-line by a third party.
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Recent Upgrade 3: Continual Learning --- Gets Smarter On-Device Without Forgetting}
\small
\begin{columns}
\begin{column}{0.58\textwidth}
\textbf{A new continual-learning stack that fights catastrophic forgetting} (inspired by hippocampal replay, Rolnick 2019):
\begin{itemize}\itemsep=0.25em
    \item \textbf{Bounded experience replay:} a fixed-memory rehearsal buffer that samples uniformly over an unbounded stream --- interleaves past examples with the live stream so the model does not wipe out what it already learned.
    \item \textbf{Continual-learning metrics:} \textbf{forgetting / retention / transfer} computed from one accuracy matrix --- ``does learning the new erase the old'' becomes \textbf{measurable}.
    \item \textbf{Streaming demo:} a head-to-head \textbf{replay vs catastrophic forgetting} run on the same stream, behaviour pinned by tests.
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{Product value:}
\begin{itemize}\itemsep=0.25em
    \item \textbf{On-device lifelong learning}: personalizes to each user locally and never forgets prior skills --- privacy stays on the edge, the model fits better the more it is used.
    \item Directly attacks the industry pain of ``fine-tuning = catastrophic forgetting'' --- a \textbf{differentiated moat} for edge personalization.
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Recent Upgrade 4: Whole-Brain Closed Loop --- A Runnable Autonomous Cognitive Agent}
\small
\textbf{A new end-to-end integration demo} (\texttt{examples/demo\_cognitive\_agent.py}) chains the brain-inspired layers into \textbf{one closed loop} solving a 2-D navigation task --- every stage is a \textbf{real module, no mocks}:
\begin{itemize}\itemsep=0.25em
    \item \textbf{Perceive} (L1 grid cells) $\rightarrow$ \textbf{Remember} (L2 spatial cognitive map; fuzzy-cue recall = pattern completion) $\rightarrow$ \textbf{Attend} (top-down goal modulation: gate CLOSED = strict no-op, OPEN = steers the backbone) $\rightarrow$ \textbf{Imagine} (L4 latent rollout, zero params, confidence decays) $\rightarrow$ \textbf{Act}.
    \item \textbf{Product value:} the leap from ``a pile of capabilities'' to an \textbf{embodied agent's cognitive loop} --- the perceive--remember--imagine--decide vision turned into \textbf{runnable} code. (Currently an integration-demo prototype, not a shipped product.)
\end{itemize}
\end{frame}

\begin{frame}{Recent Upgrade 5: From Capability to Product --- DualSpeedSentry + Production Serving}
\small
\begin{columns}
\begin{column}{0.56\textwidth}
\textbf{The DualSpeedSentry commercial loop} wires the operator layers, world model, failsafes and salience trigger into one product-grade pipeline:
\begin{itemize}\itemsep=0.2em
    \item \textbf{Perceive} (binaural acoustics: ITD/Doppler/range) $\rightarrow$ \textbf{Reason} (is the target inside the protected zone, recomputed each tick, not stored) $\rightarrow$ \textbf{Predict} (one-step ballistic forecast yields a ``surprise'') $\rightarrow$ \textbf{Trigger} (the slow/symbolic/cloud layer is \textbf{woken} only on a genuine state change, never polled on the hot loop) $\rightarrow$ \textbf{Coast} (on sensor dropout, a trust budget decides dead-reckon vs safe-stop) $\rightarrow$ \textbf{Act safely} (a circuit breaker keeps the actuator output \textbf{always finite, within mechanical limits}).
\end{itemize}
\end{column}
\begin{column}{0.44\textwidth}
\textbf{Production serving:}
\begin{itemize}\itemsep=0.2em
    \item HF-adapter \textbf{serving path} (with contract tests) + container packaging \& ops tooling --- \textbf{ready to deploy}.
\end{itemize}
\vspace{0.4em}
\textbf{Product value:}
\begin{itemize}\itemsep=0.2em
    \item A deployable \textbf{safety-critical / monitoring-sentry} vertical (fast reflex + slow deliberation, dual-speed).
    \item Provably bounded failsafes --- meeting the reliability bar of \textbf{industrial / security / defense}.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
