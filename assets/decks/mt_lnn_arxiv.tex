...
Fourth, a smaller-budget (2000-step, single-seed) reconfirmation on WikiText-103 under full fp32 training placed MT-LNN's validation perplexity below both the matched Transformer baseline and a first Mamba \citep{gu2023mamba} data point (257.5 vs 370.8 vs 414.0 respectively, at 126.0M/142.1M/129.1M parameters), consistent with the main from-scratch result above obtained at a much larger training budget. Because Mamba's hidden size and layer count are not width/depth-matched to the Transformer/MT-LNN pair, this comparison is an external reference point rather than an architecture-matched control. The same run surfaced an isolated fp16-precision robustness gap: under fp16 automatic mixed precision, MT-LNN's training loss diverged past step 629 on a separate run while the Transformer baseline trained stably for the full 2000 steps under the identical recipe; full fp32 training does not exhibit this divergence and preserves MT-LNN's perplexity advantage, but the fp16-specific root cause remains open.
% ========================================================
\section{Conclusion}
