...
MT-LNN achieves 14.7\% lower PPL than the \emph{simple-reference} Transformer of Sec.~\ref{sec:experiments} and $2.2\times$ higher $\phihat$ ($3.6\times$ higher $\phig$). This baseline is a plain post-LN/GELU Transformer; a modern RoPE/RMSNorm/SwiGLU baseline is stronger and reverses the perplexity ordering (Sec.~\ref{sec:limitations}). The GWTB contributes $\approx$0.9 PPL points independently (MT-LNN-nGWT vs.\ MT-LNN). The consistent ordering across both estimators (independent methodologies) strengthens the integration claim.
...
To test the temporal-recurrence hypothesis — whether MT-LNN's advantage holds and \emph{grows} with sequence length — we run a \textbf{selective copy} task \citep{arjovsky2016unitary}: a sequence of $T_\text{total} = T_\text{noise} + 5$ tokens contains $T_\text{noise}$ noise tokens followed by 5 ``memorable'' tokens; the model must reproduce the 5 memorable tokens and ignore all noise. Success requires tracking state across $T_\text{noise}$ positions.

All three models use $\approx$200K parameters each (2 layers, $d_\text{model}=104$, 4 heads), identical training recipes (1500 steps per length), and — critically — are decoded the \emph{same} way: a full-sequence recompute at each step. (An earlier draft decoded the cacheless baselines incrementally off a non-existent cache, dropping their context and inflating the reported advantage to $\times$42; the numbers below are the corrected, fair measurements.)

\begin{table}[h]
\centering
\caption{Selective Copy: held-out sequence-exact accuracy at three noise lengths, equal 1500-step budget, fair full-sequence decode for all models. Advantage = MT-LNN / Transformer seq-exact ratio.}
\label{tab:long_ctx}
\begin{tabular}{lccccc}
\toprule
$T_\text{total}$ & $T_\text{noise}$ & Transformer & LNN & MT-LNN & Advantage \\
\midrule
37   &  32 & 67.2\% & 70.3\% & 88.3\% & $\times$1.3 \\
101  &  96 & 57.0\% & 72.7\% & 74.2\% & $\times$1.3 \\
229  & 224 & 10.9\% & 17.2\% & 21.9\% & $\times$2.0 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding.} Under a fair decode the advantage is \emph{modest, not order-of-magnitude}: MT-LNN has the highest whole-sequence recall at every length, leading a vanilla Transformer by $\sim$17--21 points at $T=37$/$101$, and the \emph{ratio} grows to $\times$2.0 at $T=229$ as the Transformer degrades faster — although absolute accuracy collapses for all three under the limited budget at that length. The liquid-LTC baseline (LNN) is competitive throughout, even matching MT-LNN at $T=101$, so most of the gain on this task is attributable to the liquid recurrence, with the microtubule machinery contributing a smaller increment concentrated in the all-or-nothing seq-exact metric. This is suggestive of a length-scaling benefit, but the present evidence is directional rather than a clean scaling law.
...
Fourth, and most consequentially, MT-LNN's apparent perplexity advantage does not survive convergence against a modern baseline. A short-budget (2000-step, single-seed) fp32 run placed MT-LNN well below a matched Transformer (257.5 vs 370.8, with a first Mamba \citep{gu2023mamba} point at 414.0). Training the same architectures to convergence (20{,}000 steps, three seeds each, identical fp32 recipe) reverses the reading: MT-LNN reaches $88.93 \pm 0.33$ validation perplexity versus $94.14 \pm 0.78$ for the simple matched Transformer --- a residual $5.5\%$ advantage --- but a modern Transformer baseline (RoPE, RMSNorm, SwiGLU) reaches $78.86 \pm 0.25$, i.e.\ $11.3\%$ \emph{better} than MT-LNN at comparable parameter count (144.1M/126.0M/142.1M). The short-budget gap was therefore an artifact of undertraining compounded by a weak reference baseline, and we retract it as evidence of a perplexity advantage. All three architectures trained stably with no non-finite loss. We report language-modeling quality as an open gap rather than a strength: the architecture's remaining case rests on the constant-memory and cross-session recall properties, which no Transformer configuration removes. Separately, an fp16-specific robustness gap that we previously reported as unresolved has now been root-caused and fixed. Under fp16 automatic mixed precision MT-LNN's loss became non-finite mid-training (step 629 on one machine, 875--896 on another --- the step varies with data order), while the Transformer baseline trained stably under the identical recipe. Instrumenting every leaf module showed the failure was \emph{not} activation overflow: peak activations reached only $\approx$67 against fp16's 65504 ceiling. The cause was the ordering of the attention scaling inside the global-coherence layer, which computed $(QK^\top)/\sqrt{d_h}$ --- performing the $d_h{=}64$ accumulation \emph{before} the down-scaling. As training grows the query/key projections (measured peaks 52.8 and 60.8), that intermediate product reaches $\sim$2$\times10^5$ and overflows fp16 inside the matmul itself; the resulting $\mathrm{Inf}$ then meets the zeros of the causal mask in the collapse-gate energy reduction, and $\mathrm{Inf}\times 0=\mathrm{NaN}$ propagates through $\sigma(\cdot)$ to the entire layer output. Rescaling as $(Q/\sqrt{d_h})K^\top$ (algebraically identical, but with the accumulation held $\sqrt{d_h}$ lower), replacing the masked multiply with a select, and accumulating the gate energy in fp32 removes the divergence: the identical 2000-step recipe that previously failed now completes with finite loss throughout, and reaches $257.91$ validation perplexity against $257.48$ for the same recipe in fp32 --- a $0.17\%$ difference, well inside the $\pm4.89$ seed-to-seed variance we measure at this budget. The fp16 path is therefore restored to fp32 parity rather than merely made non-crashing. fp32 behaviour is unchanged. We note that every other attention surface in the architecture delegates to PyTorch's \texttt{scaled\_dot\_product\_attention}; the global-coherence layer was the single hand-rolled score computation, which is why the failure was isolated to it.
% ========================================================
\section{Conclusion}
\label{sec:conclusion}

MT-LNN is the first neural architecture to jointly instantiate: microtubule dynamics (13-protofilament MT-DL with $P\!\times\!S$ resonance, three-way lateral coupling, GTP-cap renewal, MAP-protein gates), Global Workspace Theory (GWTB), and the Anesthesia Validation Protocol — all in a single Transformer-compatible, production-grade codebase. MT-LNN achieves 14.7\% lower perplexity than a simple-reference Transformer (though not against a modern RoPE/RMSNorm/SwiGLU baseline, which leads by 11.3\% --- Sec.~\ref{sec:limitations}), $2.2\times$ higher $\phihat$ ($3.6\times$ higher $\phig$), and 89.5\%/87.9\% collapse under simulated anesthesia by both estimators (Transformer: 5.9\%). The agreement across two independent integration estimators ($\phihat$: kNN-based; $\phig$: spectral/Gaussian) cross-validates the information-integration claim. All code and weights are open-sourced at \url{https://github.com/everest-an/M1}.
