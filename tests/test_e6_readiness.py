...
⚠️ 诚实边界（2026-08-28 审查注明）：本文件 4/5 的测试是**源码字符串断言**
而非行为测试——行为验证需要真训练（≥2000 步 × 3 seeds × GPU），本机无法
承载。源码断言只钉住"旋钮存在、默认值正确、行内记录在场"，不证明训练
行为；真实验证以训练机上的 E6 运行（--good_recipe --sanity_steps 2000）
的 jsonl 行为准。
"""
